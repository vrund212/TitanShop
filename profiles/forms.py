...
class ProfileForm(forms.ModelForm):
    ...
    bio = forms.CharField(required=False, validators=[MinLengthValidator(10)], widget=forms.Textarea())
...
class CustomChangePasswordForm(PasswordChangeForm):
    ...
    def clean_new_password1(self):
        ...
        first_name = getattr(getattr(self.user, 'profile', None), 'first_name', '') or ''
        last_name = getattr(getattr(self.user, 'profile', None), 'last_name', '') or ''

        if first_name and first_name.lower() in new_password1.lower():
            raise forms.ValidationError('Your password cannot be too '
                                        'similar to your other personal '
                                        'information')

        if last_name and last_name.lower() in new_password1.lower():
            raise forms.ValidationError('Your password cannot be too '
                                        'similar to your other personal '
                                        'information')
