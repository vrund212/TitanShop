...
class Category(models.Model):
    ...
    def display_image_url(self):
        ...
        first_product = self.products.filter(available=True).order_by('id').first()
        if first_product:
            return first_product.display_image_url
        return ''
...
class Product(models.Model):
    ...
    external_source_id = models.PositiveIntegerField(blank=True, null=True, db_index=True)
...
class PromoCode(models.Model):
    ...
    discount_percent = models.IntegerField()  # Discount as percentage (e.g., 50 for 50%)
